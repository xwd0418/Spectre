...
class DecodeSampler:
  ...
  def greedy_decode(self, decode_fn, batch_size, device="cpu"):
    ...
    pad_mask = torch.zeros((self.max_seq_len, batch_size),
  ...
  def beam_decode(self, decode_fn, batch_size, device="cpu", k=5):
    ...
    tokens_list = [token_ids.transpose(0, 1).tolist()
                   for token_ids in token_ids_list]
    tokens_list = [self.tokeniser.convert_ids_to_tokens(
        tokens) for tokens in tokens_list]
    mol_strs_list = [self.tokeniser.detokenise(
        tokens) for tokens in tokens_list]
  ...
  def _update_beams_(self, i, decode_fn, token_ids_list, pad_mask_list, lls_list):
    ...
    new_lls = [self._beam_step(decode_fn, t, m, lls)
               for t, m, lls in zip(ts, ms, lls_list)]
    ...
    for new_beam_idx, (new_ids, beam_idxs, lls) in enumerate(zip(new_ids_list, beam_idxs_list, top_lls)):
      ...
      token_ids = [token_ids_list[beam_idx][:, b_idx]
      ...
      pad_mask = [pad_mask_list[beam_idx][:, b_idx]
                  for b_idx, beam_idx in enumerate(beam_idxs)]
  ...
  def _norm_length(self, seq_lls, mask):
    ...
    if self.length_norm is not None:
      ...
      norm = torch.pow(5 + seq_lengths, self.length_norm) / \
          pow(6, self.length_norm)
  ...
  def _sort_beams(mol_strs, log_lhs):
    ...
    for mols, lls in zip(mol_strs, log_lhs):
      mol_lls = sorted(zip(mols, lls), reverse=True,
                       key=lambda mol_ll: mol_ll[1])
  ...
  def calc_sampling_metrics(sampled_smiles, target_smiles):
    ...
    if data_type == str:
      results = DecodeSampler._calc_greedy_metrics(
          sampled_smiles, canon_targets)
    elif data_type == list:
  ...
  def _calc_greedy_metrics(sampled_smiles, target_smiles):
    ...
    canon_smiles = ["Unknown" if mol is None else Chem.MolToSmiles(
        mol) for mol in sampled_mols]
    correct_smiles = [target_smiles[idx] ==
                      smi for idx, smi in enumerate(canon_smiles)]
  ...
  def _calc_beam_metrics(sampled_smiles, target_smiles):
    ...
    top_1_results = DecodeSampler._calc_greedy_metrics(
        top_1_samples, target_smiles)
    ...
    for num_samples in num_samples_list:
      ...
      for batch_idx, mols in enumerate(sampled_smiles):
        ...
        samples_smiles = ["Unknown" if mol is None else Chem.MolToSmiles(
            mol) for mol in samples_mols]
        correct_smiles = [smi == target_smiles[batch_idx]
                          for smi in samples_smiles]
