...
class _AbsDataModule(pl.LightningDataModule):
  ...
  def setup(self, stage=None):
    ...
    if self.val_idxs is not None and self.test_idxs is not None:
      train_dataset, val_dataset, test_dataset = self.dataset.split_idxs(
          self.val_idxs, self.test_idxs)

    # Or randomly
    else:
      train_dataset, val_dataset, test_dataset = self.dataset.split(
          self.split_perc, self.split_perc)
...
class FineTuneReactionDataModule(_AbsDataModule):
  ...
  def _collate(self, batch, train=True):
    ...
    reacts_pad_mask = torch.tensor(
        reacts_mask, dtype=torch.bool).transpose(0, 1)
