...
class MolEncTokeniser:
  def __init__(
    ...
    Args:
        vocab (List[str]): Vocabulary for tokeniser
        chem_token_idxs (List[int]): List of idxs of chemical tokens
        prog (re.Pattern): Regex object for tokenising
  ...
  def from_vocab_file(
    ...
    token_idxs = [pad_token_idx, unk_token_idx, begin_token_idx,
                  end_token_idx, mask_token_idx, sep_token_idx]
  ...
  def tokenise(self, sents1, sents2=None, mask=False, pad=False):
    ...
    sent_masks = [[0] + mask + [1]
                  for mask in sent_masks] if sent_masks is not None else None
    ...
    if pad:
      ...
      sent_masks, _ = self._pad_seqs(
          sent_masks, False) if sent_masks is not None else (None, None)
  ...
  def _concat_sentences(self, tokens1, tokens2, sep):
    ...
    sent_masks = [([0] * len(ts1)) + [0] + ([1] * len(ts2))
  ...
  def _mask_replace(self, ts):
    ...
    masked = [self._mask_token(ts[i]) if m else ts[i]
