...
class GenericIndexedDataset(Dataset):
    ...
    def __init__(self, dir, features, split="train"):
        self.dir = dir
        assert(split in ["train", "val", "test"])

        self.split_dir = dir / split
        ...
        for feature in features:
            if os.path.exists(self.splitdir / feature / "index.pkl"):
                with open(self.splitdir / feature / "index.pkl", "rb") as f:
                   feats = pickle.load(f)
    ...
    def __getitem__(self, idx):
        ...
        for idx, feature in enumerate(self.features):
            if feature not in self.indexed_features:
                feats[idx] = torch.load(self.split_dir / feature / f"{idx}.pt")
            else:
                feats[idx] = self.indexed_features[feature][idx]
        return feats

def build_collate_fn(feature_handlers):
    def collate(batch):
      feat_columns = zip(*batch)
...
class GenericIndexedModule(pl.LightningDataModule):
    def __init__(self, dir, features, feature_handlers, batch_size: int = 32, num_workers=4):
        super().__init__()
        self.dir = dir
        self.features = features
    ...
    def setup(self, stage):
        if stage == "fit" or stage is None:
            self.train = GenericIndexedDataset(self.dir, self.features, split="train")
            self.val = GenericIndexedDataset(self.dir, self.features, split="val")
        if stage == "test":
            self.test = GenericIndexedDataset(self.dir, self.features, split="test")
        if stage == "predict":
