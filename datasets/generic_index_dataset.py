...
class GenericIndexedDataset(Dataset):
  ...
  def __init__(self, direc, features, split="train", len_override=None):
    ...
    if self.num_data is None:
      raise Exception(
          f"[{str(self.__class__)}]: Could not interpolate dataset size!!!")
...
class GenericIndexedModule(pl.LightningDataModule):
  ...
  def setup(self, stage):
    if stage == "fit" or stage is None:
      self.train = GenericIndexedDataset(
          self.dir, self.features, split="train", len_override=self.len_override)
      self.val = GenericIndexedDataset(
          self.dir, self.features, split="val", len_override=self.len_override)
    if stage == "test":
      self.test = GenericIndexedDataset(
          self.dir, self.features, split="test", len_override=self.len_override)
    if stage == "predict":
