...
class PairDataset(Dataset):
    def __init__(self, split="train"):
        ...
        self.new_specs = os.path.join(self.dir, "new_specs_05_02_22")
        assert(os.path.exists(self.orig_hsqc) and os.path.exists(self.new_specs))
        assert(split in ["train", "val", "test"])
        self.files = list(os.listdir(os.path.join(self.new_specs, split)))
    def __len__(self):
        return len(self.files)

    def __getitem__(self, i):
        hsqc = np.load(os.path.join(self.dir, "data", self.split, "HSQC_2ch", self.files[i]))
        ms = np.load(os.path.join(self.dir, "new_specs_05_02_22", self.split, self.files[i]))
...
class PairDataModule(pl.LightningDataModule):
    ...
    def val_dataloader(self):
        return DataLoader(self.val, shuffle=True, batch_size=self.batch_size, collate_fn=pad, num_workers=4)
